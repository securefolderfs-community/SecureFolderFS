...
{
    [ObservableObject]
    public sealed partial class MigratorV3_V4 : UserControl, IMigratorControl
    {
        ...
        {
            // Unlike the V2 to V3 migration, the keystore is carried over unchanged, so recovering
            // does not require new credentials to be configured for the vault
            if (string.IsNullOrEmpty(RecoveryOverlayViewModel?.RecoveryKey))
                return;
        ...
        public static readonly DependencyProperty MigrateCommandProperty =
            DependencyProperty.Register(nameof(MigrateCommand), typeof(ICommand), typeof(MigratorV3_V4), new PropertyMetadata(null));
        ...
        public static readonly DependencyProperty RecoverCommandProperty =
            DependencyProperty.Register(nameof(RecoverCommand), typeof(ICommand), typeof(MigratorV3_V4), new PropertyMetadata(null));
        ...
        public static readonly DependencyProperty VaultNameProperty =
            DependencyProperty.Register(nameof(VaultName), typeof(string), typeof(MigratorV3_V4), new PropertyMetadata(null));
        ...
        public static readonly DependencyProperty VaultFolderProperty =
            DependencyProperty.Register(nameof(VaultFolder), typeof(IFolder), typeof(MigratorV3_V4), new PropertyMetadata(null,
                async (s, e) =>
                {
                    if (s is not MigratorV3_V4 migratorV3V4)
                        return;

                    if (e.NewValue is not IFolder vaultFolder)
                        return;

                    await migratorV3V4.BeginAuthenticationAsync(vaultFolder);
                }));
        ...
        public static readonly DependencyProperty ProvideContinuationButtonProperty =
            DependencyProperty.Register(nameof(ProvideContinuationButton), typeof(bool), typeof(MigratorV3_V4), new PropertyMetadata(true));
    }
}
